...
Axdyn = data.Ax-movmean(data.Ax,sample_rate_sensors);
Aydyn= data.Ay-movmean(data.Ay,sample_rate_sensors);
Azdyn = data.Az-movmean(data.Az,sample_rate_sensors);
...
for i=1:height(labellist)
    ...
    if(isfield(segmentsPerLabel,fieldname))
        ...
        segmentsPerLabel.(fieldname) = addSegment(segmentsPerLabel.(fieldname),data(from:to,:),max_seg_length,fieldname);   
    else
        % No, add new field
        ret =  addSegment({},data(from:to,:),max_seg_length,fieldname);
