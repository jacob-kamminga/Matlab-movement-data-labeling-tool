function [featureTable,loss] = calcFeatures(inputTable,seg_idx,ws,sampling_rates,overlap,selected_features,selected_columns)
...
% fvtool(lpFilt);
% res=[];
res = num2cell(zeros(floor(height(inputTable)/(window_size*overlap)),size(selected_columns,2)*size(selected_features,2)+1));
header={'label'};
% loop over segments
